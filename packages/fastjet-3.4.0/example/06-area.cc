...
int main(){
  ...
  // label the columns
  printf("%5s %15s %15s %15s %15s %15s %15s\n","jet #", "rapidity", "phi", "pt", "area", "area error","# constituents");
 
  // print out the details for each jet
  for (unsigned int i = 0; i < inclusive_jets.size(); i++) {
    //Let's see how many constituents are in the jets
    vector<fastjet::PseudoJet> constituents = inclusive_jets[i].constituents();

    printf("%5u %15.8f %15.8f %15.8f %15.8f %15.8f %10u\n", i,
	   inclusive_jets[i].rap(), inclusive_jets[i].phi(), inclusive_jets[i].perp(),
	   inclusive_jets[i].area(), inclusive_jets[i].area_error(), (unsigned int) constituents.size());
  }
