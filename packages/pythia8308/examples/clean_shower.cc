...
void get_bjets(std::vector<fastjet::PseudoJet>& b_jets, std::vector<fastjet::PseudoJet>& vec, fastjet::JetDefinition& jetDef){


    fastjet::ClusterSequence clust_seq(vec, jetDef);
    ...
    std::vector<fastjet::PseudoJet> sorted_jets    = sorted_by_pt(inclusive_jets);
    ...
    for (const auto &jet : sorted_jets) {
        ...
        if (is_b_jet){
            ...
            double zcut = 0.05;
            double rcut_factor = 0.2;
            fastjet::Pruner pruner(fastjet::antikt_algorithm, zcut, rcut_factor);
            fastjet::PseudoJet groomed_jet = pruner(jet);
            b_jets.push_back(groomed_jet);

            num_b_jets++;
        }
    }
         
    if(num_b_jets < 2){                 //Here an unphysical jet is added to keep the kinematic vectors the same size and events with these jets are later deleted
    ...
    // if(num_b_jets == 1){
    // //experiment of putting the grooming of the jets in the getting jets object

    //     double zcut = 0.1;
    //     double rcut_factor = 0.5;
    //     fastjet::Pruner pruner(fastjet::antikt_algorithm, zcut, rcut_factor);

    //     cout << "\nOriginal jets that will be groomed: " << endl;
    //     for (vector<fastjet::PseudoJet>::iterator jit=jets.begin(); jit!=jets.end(); jit++){
    ...
    //     cout << "\nUsing Pruner: " << pruner.description() << endl;
    //     for (vector<fastjet::PseudoJet>::iterator jit=jets.begin(); jit!=jets.end(); jit++){
    //         const fastjet::PseudoJet & c = *jit;

    //         fastjet::PseudoJet j = pruner(c);
    //         b_jets.push_back(j);
    //         cout << "for loop works2" << endl;

    //         int n_rejected;
    //         const fastjet::Pruner::StructureType & fj_struct = j.structure_of<fastjet::Pruner>();
    ...
    //             << "  [kept: " << j.pieces().size()
    //             << ", rejected: " << n_rejected
    //             << ", Rcut: " << j.structure_of<fastjet::Pruner>().Rcut()
    //             << "]" << endl;
    //     }
    // }
...
std::pair<std::vector<fastjet::PseudoJet>,std::vector<fastjet::PseudoJet>> separate_visible_and_neutrino(std::vector<fastjet::PseudoJet>& final_particles_){
    ...
    std::vector<int> neutrino_ids = {12,14,16};
...
void process_pseudojets(std::vector<fastjet::PseudoJet>& vec1, std::vector<fastjet::PseudoJet>& vec2 = get_default_pseudojet(),
    ...
    if (kinematic_l_data_map.find(type) != kinematic_l_data_map.end()){
        ...
        kld.abs_diff_phi->push_back(abs(sorted_jets1[0].phi() - sorted_jets2[0].phi()));
    ...
    if (kinematic_cut_data_map.find(type) != kinematic_cut_data_map.end()){
        ...
        // : v_pt_miss(&v_pt_miss_), n_pt_miss(&n_pt_miss_), ll_delta_R(&ll_delta_R_), lj_delta_R(&lj_delta_R_), jj_delta_R(&jj_delta_R_) {}
...
int main(){
    ...
    //final pseudojets
    std::vector<fastjet::PseudoJet> final_particles, b_quarks, leptons, anti_leptons, b_jets, visible_particles;
    ...
    std::vector<std::pair<std::tuple<std::reference_wrapper<std::vector<fastjet::PseudoJet>>, std::reference_wrapper<std::vector<fastjet::PseudoJet>>,
                          std::reference_wrapper<std::vector<fastjet::PseudoJet>>, std::reference_wrapper<std::vector<fastjet::PseudoJet>>>, std::string>> pre_processed_particles = {
  ...
  // Begin event loop; generate until none left in input file.
    for (int iEvent = 0; iEvent  < 10 ; ++iEvent) {                // you can add ; iEvent < 10 ; for fast itteration times
    ...
    get_bjets(b_jets,visible_particles, jetDef);
    ...
    //code to analyse the situations where the b_jet has a higher pt than the hard particle.
    if ((pt_b_trailing[iEvent]- h_pt_b_trailing[iEvent] > 30) && pt_b_trailing[iEvent] > 300 && pt_b_trailing[iEvent] != 9999 && pt_b_leading[iEvent] != 9999){
        // if(iEvent == 9 || iEvent ==99 || iEvent == 999 || iEvent == 9999){
        if(list_event < 10){
            cout << "iEvent = " << iEvent << endl;
            cout << "pt_b_trailing = " << pt_b_trailing[iEvent] << endl;
            cout << "h_pt_b_trailing = " << h_pt_b_trailing[iEvent] << endl;
            pythia.process.list();
            pythia.event.list();
            list_event++;
        }
  ...
  pythia.stat();
    ...
    int total_potential_cut_count = l_pt_lead_cut_count + l_pt_trail_cut_count + b_pt_cut_count + b_eta_cut_count + ll_m_cut_count + delta_R_cut_count + vis_pt_miss_cut_count + faulty_jet_pt_cut_count;
  ...
  // Open output file
  std::ofstream hard_outfile("kinematics_vector_hard_no_nu_with_prun.csv");
  std::ofstream outfile("kinematics_vector_with_no_nu_with_prun.csv");
    ...
    // name_vector = {"pt_b_leading", "pt_l1", "pt_l2", "pt_l_leading", "pt_l_trailing", "eta_l1", "eta_l2", "eta_l_leading", "eta_l_trailing",
    //                "pt_ll", "m_ll", "DeltaPhi_ll", "DeltaEta_ll", "pt_b_leading", "pt_b_trailing", "eta_b_leading", "eta_b_trailing", "pt_bb", "m_bb"
    //               };
    ...
    //     //Apply event selection
    // for (auto& kin_vec : hard_kinematics_vector){
    //     std::vector<double> filtered_vec;
    //     for (int i = 0; i < kin_vec.size(); i++){
    //         if(event_selection[i]){
    //             filtered_vec.push_back(kin_vec[i]);
    ...
    if (kinematics_vector.size() == name_vector.size()) {
        for (size_t i = 0; i < kinematics_vector.size(); ++i) {
            std::vector<double>& kin_vec = kinematics_vector[i];
            const std::string& name = name_vector[i];
            std::cout << "size " << name << " = " << kin_vec.size() << std::endl;
        }
    } else {
        std::cout << "Error: The kinematics_vector and name_vector have different sizes." << std::endl;
    }
    ...
    if (hard_kinematics_vector.size() == name_vector.size()) {
        for (size_t i = 0; i < hard_kinematics_vector.size(); ++i) {
            ...
            const std::string& name = name_vector[i];
            std::cout << "size " << name << " hard = " << kin_vec.size() << std::endl;
        }
    } else {
        std::cout << "Error: The kinematics_vector and name_vector have different sizes." << std::endl;
    }
    ...
    if (hard_kinematics_vector.size() == name_vector.size()) {
        ...
        std::cout << "Error: The hard_kinematics_vector and name_vector have different sizes." << std::endl;
    }
