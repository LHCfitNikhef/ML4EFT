...
#pragma endregion resize
    ...
    // std::vector<double> pt_l1_diff, pt_l2_diff,pt_b_lead_diff,pt_b_trail_diff;

    // for(int i = 0; i < h_pt_l1.size(); i++){
    //     pt_l1_diff.push_back((std::abs(h_pt_l1[i] - pt_l1[i]))/(h_pt_l1[i] + pt_l1[i]));
    //     pt_l2_diff.push_back((std::abs(h_pt_l2[i] - pt_l2[i]))/(h_pt_l2[i] + pt_l2[i]));
    //     pt_b_lead_diff.push_back((std::abs(h_pt_b_leading[i] - pt_b_leading[i]))/(h_pt_b_leading[i] + pt_b_leading[i]));
    //     pt_b_trail_diff.push_back((std::abs(h_pt_b_trailing[i] - pt_b_trailing[i]))/(h_pt_b_trailing[i] - pt_b_trailing[i]));
    // }

    // hard_kinematics_vector = {pt_l1_diff, pt_l2_diff,pt_b_lead_diff,pt_b_trail_diff};
    // kinematics_vector = {pt_l1_diff, pt_l2_diff,pt_b_lead_diff,pt_b_trail_diff};
    // name_vector = {"pt_l1_diff","pt_l2_diff","pt_b_lead_diff","pt_b_trail_diff"};

    std::vector<double> pt_l1_reldiff, pt_l2_reldiff,pt_b_lead_reldiff,pt_b_trail_reldiff, pt_l_lead_reldiff, pt_l_trail_reldiff;

    for(int i = 0; i < h_pt_l1.size(); i++){
        pt_l1_reldiff.push_back(-(h_pt_l1[i] - pt_l1[i])/(h_pt_l1[i]));
        pt_l2_reldiff.push_back(-((h_pt_l2[i] - pt_l2[i]))/(h_pt_l2[i]));
        pt_l_lead_reldiff.push_back(-((h_pt_l_leading[i] - pt_l_leading[i]))/(h_pt_l_leading[i] ));
        pt_l_trail_reldiff.push_back(-((h_pt_l_trailing[i] - pt_l_trailing[i]))/(h_pt_l_trailing[i]));
        pt_b_lead_reldiff.push_back(-((h_pt_b_leading[i] - pt_b_leading[i]))/(h_pt_b_leading[i]));
        pt_b_trail_reldiff.push_back(-((h_pt_b_trailing[i] - pt_b_trailing[i]))/(h_pt_b_trailing[i]));
    }

    hard_kinematics_vector = {pt_l1_reldiff, pt_l2_reldiff, pt_l_lead_reldiff, pt_l_trail_reldiff ,pt_b_lead_reldiff,pt_b_trail_reldiff};
    kinematics_vector = {pt_l1_reldiff, pt_l2_reldiff,pt_l_lead_reldiff, pt_l_trail_reldiff, pt_b_lead_reldiff,pt_b_trail_reldiff};
    name_vector = {"pt_l1_reldiff","pt_l2_reldiff","pt_l_lead_reldiff", "pt_l_trail_reldiff", "pt_b_lead_reldiff","pt_b_trail_reldiff"};
