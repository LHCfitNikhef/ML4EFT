...
runcard_loc = "/data/theorie/pherbsch/ML4EFT/thesis_pim/code/cluster/shower/runcards/shower_settings.json"
...
for job_dir in job_dirs:
    ...
    hard_logs_dir = os.path.join(hard_dest_dir, 'logs')
    event_logs_dir = os.path.join(event_dest_dir, 'logs')
    os.makedirs(hard_logs_dir, exist_ok=True)
    ...
    # Modifying the log_dest_file path, incorporating job number minus one
    event_log_dest_file = os.path.join(event_logs_dir, f'log_{job_number - 1}.txt')
    hard_log_dest_file = os.path.join(hard_logs_dir, f'log_{job_number - 1}.txt')

    with open(event_log_dest_file, 'w') as log_file: 
        log_file.write(output.stdout.decode('utf-8'))

    with open(hard_log_dest_file, 'w') as log_file: 
        log_file.write(output.stdout.decode('utf-8'))

    if output.returncode != 0:
        error_dest_file = output_directory + "/error_" + dir_name + str(job_number) + '.txt'
