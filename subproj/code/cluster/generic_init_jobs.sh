...
while IFS= read -r coefficient || [ -n "$coefficient" ]; do
    ...
    qsub -q smefit -W group_list=smefit -l nodes=1:ppn=4 -l vmem=32gb -l walltime=00:15:00 -o "${cluster_message_output_dir}" -e "${cluster_message_err_dir}" -v "coefficient=$coefficient,zcut_beta=$zcut_beta" job_list.sh
