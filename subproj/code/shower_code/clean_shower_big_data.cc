...
void get_bjets(std::vector<fastjet::PseudoJet> &b_jets, std::vector<fastjet::PseudoJet> &vec, fastjet::JetDefinition &jetDef, double Rparam = 0)
{
...
#pragma endregion resize
        ...
        get_bjets(b_jets, visible_particles, jetDef, Rparam);
    ...
    //the output folders are given by the python script that calls this program
    ...
    std::string hard_outfile_name = output_folder_hard + "events_" + std::to_string(job_number -1) + ".csv";
    std::string outfile_name = output_folder_event + "events_" +  std::to_string(job_number -1) + ".csv";
   ...
   //consistency check in output kinematic vector size
    ...
    if (kinematics_vector.size() == name_vector.size())
    {
        ...
        {
            if (kinematics_vector[i].size() != kinematics_vector[0].size())
            {
                std::cerr << "The kinematic vectors at event level don't have the same size" << std::endl;
                return 1;
            }
        }
        std::cout << "Number of events at event level = " << kinematics_vector[0].size() << std::endl;
    }
    else
    {
    ...
    if (hard_kinematics_vector.size() == name_vector.size())
    {
        ...
        {
            if (hard_kinematics_vector[i].size() != hard_kinematics_vector[0].size())
            {
                std::cerr << "The kinematic vectors at event level don't have the same size" << std::endl;
                return 1;
            }
        }
        std::cout << "Number of events at hard process level = " << hard_kinematics_vector[0].size() << std::endl;
    }
    else
    {
        std::cerr << "Error: The kinematics_vector and name_vector have different sizes." << std::endl;
