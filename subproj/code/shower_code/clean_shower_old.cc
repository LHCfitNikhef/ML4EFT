...
void get_bjets_and_leftovers(std::vector<fastjet::PseudoJet> &b_jets, std::vector<fastjet::PseudoJet> &visible_particles, std::vector<fastjet::PseudoJet> &leftover_particles, fastjet::JetDefinition &jetDef, double zcut = 0, double beta = 0, double Rparam = 0)
{
    ...
    int num_b_jets = 0;
    std::vector<fastjet::PseudoJet> b_jet_constituents;
    // B-tagging
    for (const auto &jet : sorted_jets)
    {
        ...
        if (is_b_jet)
        {
            ...
            b_jet_constituents.insert(b_jet_constituents.end(), constituents.begin(), constituents.end());

            num_b_jets++;
        }
    }

    // Get leftover particles by excluding b_jet constituents from all particles
    for (const auto &particle : visible_particles)
    {
        if (std::find(b_jet_constituents.begin(), b_jet_constituents.end(), particle) == b_jet_constituents.end())
        {
            leftover_particles.push_back(particle);
        }
    }
...
#pragma region object definitions
    ...
    std::vector<fastjet::PseudoJet> leftover_particles;
...
#pragma endregion resize
        ...
        //get the b jets
        get_bjets_and_leftovers(b_jets, visible_particles, leftover_particles, jetDef, zcut, beta, Rparam);
        ...
        for (size_t i = 0; i < pt_values.size(); ++i){
                if(pt_values[i] > 300.0){
                    cout << "external id: " << id_values[i] << " pt: " << pt_values[i] << endl;
