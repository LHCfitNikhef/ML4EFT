...
void get_bjets(std::vector<fastjet::PseudoJet> &b_jets, std::vector<fastjet::PseudoJet> &vec, fastjet::JetDefinition &jetDef, double zcut = 0, double beta = 0, double Rparam = 0)
{
    ...
    // double R = 0.40;
    int n = 4;
    fastjet::contrib::RecursiveSoftDrop rec_softdrop(beta, zcut, n, Rparam);
    ...
    {
        fastjet::PseudoJet groomed_jet = softdrop(jet);
        groomed_jets_softdrop.push_back(groomed_jet);
    }

    std::vector<fastjet::PseudoJet> sorted_jets = sorted_by_pt(groomed_jets);
    std::vector<fastjet::PseudoJet> sorted_jets_softdrop = sorted_by_pt(groomed_jets_softdrop);

    // code to debug the grooming methode
    //  cout << "pt leading jet: " << sorted_jets1[0].pt() << endl;
    //  cout << "pt leading groomed jet softdrop: " << sorted_jets_softdrop[0].pt() << endl;
    //  cout << "pt leading groomed jet: " << sorted_jets[0].pt() << endl;
    ...
    for (const auto &jet : sorted_jets)
    {
        ...
        if (is_b_jet)
        {
            ...
            // cout << "number of constituents in b jet after rec softdrop: " << jet_constituents.size() << endl;
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " zcut1 zcut2 ..." << std::endl;
        return 1;
    }

    //The  input in this case are the smeft coefficient I want to input

    string event_name = argv[1];
    // for (int i = 2; i < argc; ++i) {
    //     double zcut = std::strtod(argv[i], nullptr);
    //     string zcut_value = argv[i];
...
#pragma region pythia setup
    ...
    string lhe_file_path = "/data/theorie/jthoeve/ML4EFT_events/" + event_name + "/job1/Events/run_01/unweighted_events.lhe";
...
#pragma endregion resize
        ...
        get_bjets(b_jets, visible_particles, jetDef, zcut, beta, Rparam);
    ...
    // std::string output_folder_hard = "../output/hard/";
    // std::string output_folder_event = "../output/event/";
    std::string output_folder_hard = "../random_data_bin/hard/";
    std::string output_folder_event = "../random_data_bin/event/";
    ...
    // std::string hard_outfile_name = output_folder_hard + event_name + ".csv";
    // std::string outfile_name = output_folder_event + event_name + ".csv";
    std::string hard_outfile_name = output_folder_hard + event_name + ".csv";
    std::string outfile_name = output_folder_event + event_name + ".csv";
