...
class EventDataset(data.Dataset):
    ...
    def load_events(self):
        ...
        path_eft_dict = {'5': 'lo_1000/eft_5.lhe', '10': 'lo_1000/eft_10.lhe', '15': 'lo_1000/eft_15.lhe'}
        path_sm_dict = {'5': 'lo_1000/sm_5.lhe', '10': 'lo_1000/sm_10.lhe', '15': 'lo_1000/sm_15.lhe' }
...
def train_classifier(path, train_dataset, val_dataset):
    model = Predictor(num_inputs=1, num_hidden=30, num_outputs=1)
...
def make_predictions(network_path, train_dataset):
    loaded_model = Predictor(num_inputs=1, num_hidden=30, num_outputs=1)
    loaded_model.load_state_dict(torch.load(network_path))

    fig = plt.figure()
    mt = 172
    ax1 = fig.add_axes([0.15, 0.35, 0.75, 0.55], xticklabels=[], xlim=(1000, 2500))



    mtt = torch.arange(1000, 2500, 10).unsqueeze(1)
    x = (mtt - train_dataset.mean) / train_dataset.std
    ctG = 10
    output = [1/(1+1+ctG*loaded_model.n_alpha(x_i.float()).item()**2) for x_i in x]
    # output = [loaded_model.n_alpha(x_i.float()).item()**2 for x_i in x]

    mtt_eft = train_dataset.data_eft['10'][0].numpy()
    mtt_sm = train_dataset.data_sm['10'][0].numpy()
    #plt.scatter(mtt_eft, np.zeros(len(mtt_eft)), label='EFT')
    #plt.scatter(mtt_sm, np.ones(len(mtt_sm)), label='SM')
    ax1.plot(mtt.numpy(), output, label='NN prediction')
    x, y = ExS.plot_likelihood_ratio_1D(1, 2.5, 10)
    ax1.plot(np.array(x)*10**3, y, '--', c='red', label='Analytical result')
    ...
    ax3.plot(np.array(x)*10**3, np.array(y)/np.array(output))
    plt.ylabel('ana/NN')
    plt.xlim((1000, 2500))
