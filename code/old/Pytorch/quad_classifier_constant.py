...
def loss_constant(c, a, label, w_e):
    # outputs.shape = (batch_size, 1), output \in (0, 1)
    # labels.shape = (batch_size, 1), labels \in {0, 1}
    # w_e.shape = (batch_size, 1), w_e \in [0, \infty)
    f_const = 1/(2+c*a)
    loss = (1 - label)*w_e*f_const**2 + label*w_e*(1 - f_const)**2
    # add up all the losses in the batch
    return torch.sum(loss, dim = 0)
...
def main(trained, path):
    ...
    weight_eft = train_dataset.data_eft['10'][1]
    label_eft = train_dataset.data_eft['10'][2]

    weight_sm = train_dataset.data_sm['10'][1]
    label_sm = train_dataset.data_sm['10'][2]

    weight_10 = torch.cat((weight_eft, weight_sm))
    label_10 = torch.cat((label_eft, label_sm))

    weight_eft = train_dataset.data_eft['5'][1]
    label_eft = train_dataset.data_eft['5'][2]

    weight_sm = train_dataset.data_sm['5'][1]
    label_sm = train_dataset.data_sm['5'][2]

    weight_5 = torch.cat((weight_eft, weight_sm))
    label_5 = torch.cat((label_eft, label_sm))


    fig = plt.figure()
    a_list = np.arange(0.15, 0.45, 0.001)
    loss_list = [loss_constant(10, a_i, label_10, weight_10) for a_i in a_list]
    print(a_list[np.argmin(loss_list)])
    plt.plot(a_list, loss_list)
    plt.xlabel('A')
    plt.ylabel('L(A)')
    plt.title('Loss with a constant r(x,c) = 1 + cA')
    plt.show()
    fig.savefig('constant_ratio_loss.pdf')
    sys.exit()

    # train_dataset.resc = True
    # val_dataset.resc = True

    visualize([train_dataset.data_eft_['30'][0], train_dataset.data_eft['10'][0], train_dataset.data_sm['10'][0]], [30, 10, 0])
