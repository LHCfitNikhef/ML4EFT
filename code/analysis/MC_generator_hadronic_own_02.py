...
def weight(sqrts, mu, x1, x2, theta):
    ...
    w_ii = dsigmadThetaggSM(sqrts, theta) * ( p.xfxQ(21, x1, mu) *  p.xfxQ(21, x2, mu) )     
    #w_ii += dsigmadThetaqqEFT(sqrts,theta, 1)*( p.xfxQ(-1, x1, mu) *  p.xfxQ(1, x2, mu) +p.xfxQ(-2, x1, mu) *  p.xfxQ(2, x2, mu) +p.xfxQ(-3, x1, mu) *  p.xfxQ(3, x2, mu) +p.xfxQ(-4, x1, mu) *  p.xfxQ(4, x2, mu) +p.xfxQ(-5, x1, mu) *  p.xfxQ(5, x2, mu) )
...
for ii in range(0,N):
    ...
    tau_ii = tau_min + random.random()*deltatau
    ...
    w_ii = weight(np.sqrt(hats), mu, x1, x2, th_ii) * deltath * deltay * deltatau  / (x1 * x2)
    # add to the sums
    sum_w = sum_w + w_ii
