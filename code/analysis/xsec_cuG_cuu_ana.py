...
class crossSectionSMEFT:
    ...
    def sigma_part_qq(self, hats, cuGRe, cuu):
        ...
        sqrt = np.sqrt(1 - 4 * mt ** 2 / hats)
        kappa_11 = sqrt * (8 * np.pi * v ** 2 * yt ** 2 * asQCD * (8 * mt ** 2 + hats)) / (108 * np.pi * LambdaSMEFT ** 4 * hats)
...
def weight(sqrts, mu, x1, x2, cuGRe, cuu):
    ...
    w_ii += (xsec.sigma_part_qq(hats, cuGRe, 0)) * (p.xfxQ(1, x1, mu) * p.xfxQ(-1, x2, mu) + p.xfxQ(3, x1, mu) * p.xfxQ(-3, x2, mu) + p.xfxQ(5, x1, mu) * p.xfxQ(-5, x2, mu))
    w_ii += (xsec.sigma_part_qq(hats, cuGRe, cuu)) * (p.xfxQ(2, x1, mu) * p.xfxQ(-2, x2, mu) + p.xfxQ(4, x1, mu) * p.xfxQ(-4, x2, mu))
...
def plotData(binWidth, mtt_max, cuGRe, cuu):
    ...
    plt.title('Top-pair production in SMEFT with '+ r'$c_{tG}$' + ' and ' + r'$c_{tt}$')
    ...
    plt.show()
    fig.savefig('mg5_ana_ctG_cuu.pdf')
