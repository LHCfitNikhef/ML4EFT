...
with open("z_scores/binned/bin_2_v3/z_scores_mcuu.dat", "r") as f:
...
with open("z_scores/binned/bin_2_v3/z_scores_mcuu_v5.dat", "r") as f:

    reader = csv.reader(f, delimiter='\t')
    z_scores = []
    p_value_bin_2_v2 = []
    p_value_bin_2_unc_v2 = []
    ...
    for c in cuu:
        ...
        p_value_c = np.mean(p_values_c)
        p_value_c_unc = np.std(p_values_c) / np.sqrt(len(p_values_c))
        p_value_bin_2_v2.append(p_value_c)
        p_value_bin_2_unc_v2.append(p_value_c_unc)
...
# with open("z_scores/binned/bin_2_v3/z_scores_mcuu_v4.dat", "r") as f:
#
#     reader = csv.reader(f, delimiter='\t')
#     z_scores = []
#     z_scores_unc = []
...
#     for line in reader:
#         z_scores.append([float(line[1]), float(line[2])])
#         z_scores_unc.append([float(line[1]), float(line[3])])
#     z_scores = np.array(z_scores)
#     z_scores_unc = np.array(z_scores_unc)
#
#
#
#     for c in cuu:
#         z_scores_c = z_scores[z_scores[:, 0] == c][:, 1]
#         z_scores_c_unc = z_scores_unc[z_scores_unc[:, 0] == c][:, 1]
#
#         z_score = np.sum(z_scores_c/z_scores_c_unc**2)/np.sum(1/z_scores_c_unc**2)
#         z_score_unc = np.sqrt(1/np.sum(1/z_scores_c_unc**2))
#         p_value_c = 1 - norm.cdf(z_score)
#         p_value_c_unc = normal(z_score)*z_score_unc
#
#         # p_values_c = 1 - norm.cdf(z_scores_c)
#         # p_values_c_unc = normal(z_scores_c)*z_scores_c_unc
#         # p_value_c = np.sum(p_values_c/p_values_c_unc**2)/np.sum(1/p_values_c_unc**2)#np.mean(p_values_c)
#         # p_value_c_unc = np.sqrt(1/np.sum(1/p_values_c_unc**2))#np.std(p_values_c) / np.sqrt(len(p_values_c))
#         p_value_bin_2.append(p_value_c)
#         p_value_bin_2_unc.append(p_value_c_unc)
...
ax.errorbar(cuu, p_value_bin_2, yerr=p_value_bin_2_unc, fmt='.', capsize=3,
            color='C1', label=r'$\rm{p-value\;(bin\;2\;run\;I)}$')

ax.errorbar(cuu, p_value_bin_2_v2, yerr=p_value_bin_2_unc_v2, fmt='.', capsize=3,
            color='C0', label=r'$\rm{p-value\;(bin\;2\;run\;II)}$')
#
...
popt_bin_2_v2, _ = curve_fit(decay_exp, cuu, p_value_bin_2_v2, sigma=p_value_bin_2_unc_v2)
#popt_bin_3, _ = curve_fit(quad_pol, cuu, p_value_bin_3, sigma=p_value_bin_3_unc)


x = np.linspace(-0.85, -0.35, 400)
plt.hlines(0.05, -0.85, -0.35, color='black', linestyle='dashed')
...
ax.plot(x, decay_exp(x, *popt_bin_2), color='C0', linestyle='dotted')#, label=r'$\rm{Exp\;fit\;(bin\;2)}$')
ax.plot(x, decay_exp(x, *popt_bin_2_v2), color='C1', linestyle='dotted')#, label=r'$\rm{Exp\;fit\;(bin\;2)}$')
...
idx_bin_2_v2 = np.argwhere(np.diff(np.sign(decay_exp(x, *popt_bin_2_v2) - 0.05))).flatten()
...
plt.plot(x[idx_bin_2_v2], decay_exp(x[idx_bin_2_v2], *popt_bin_2_v2), 'kx')
...
plt.tight_layout()
#plt.show()
plt.savefig('p_value_scan_mcuu_bin_2_comp.pdf')
