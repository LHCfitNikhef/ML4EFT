...
##### TRUTH ########
with open("z_scores/truth/z_scores_pdiag.dat", "r") as f:
...
x = np.linspace(0, 2*np.max(c_dist), 400)
...
ax.set_xlabel(r'$\rm{\tilde{c}}$', fontsize=20)
#ax.set_xlim((0.38, 1.42))
ax.set_xlim((0.05, 2*np.max(c_dist)))
ax.set_ylim((0, 0.45))
