...
c
      ...
      REAL*8 pt, PtDot, rap, SumDot
      external pt, PtDot, rap, SumDot
...
C       CMS tt -> bb vl vl~ l+ l- conditional cuts

      if (SumDot(p(0, 5),p(0,6),1.d0) .lt. 20.0d0**2) then
            dummy_cuts=.false.
      else if ((SumDot(p(0, 5),p(0,6),1.d0) .gt. 76.0d0**2 .and. (SumDot(p(0, 5),p(0,6),1.d0) .lt. 106.0d0**2))) then
             dummy_cuts=.false.
      endif
