...
def sigma_part_vh(hats, cHW, cHq3, lin, quad):
    ...
    if quad:
        ...
        xsec_quad_cHW_cHW = (cth2 ** 2 * mz ** 2 * pz * (3 * mz ** 2 + 2*pz2)*np.sqrt(hats)*(9-24*sth2 +32*sth2**2))/(81*np.pi *(mz ** 2 - hats)**2)

        xsec_quad_cHW_cHq3 = -(2 * cth2 * mz ** 4 * pz * np.sqrt(mz ** 2 + pz2) * (-3 + 4 * sth2))/(9*np.pi*(mz**2-hats)**2)
...
def findCoeff(bins):
    ...
    cHW_cHq3 = (sigma_bin(bins, cHW=10, cHq3=10, lin=False, quad=True) - (sm + 10 * cHW_lin + 10**2 * cHW_quad + 10 * cHq3_lin + 10**2 * cHq3_quad)) / 10**2
