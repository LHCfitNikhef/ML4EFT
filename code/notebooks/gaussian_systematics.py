...
contour_data = np.array(contours.allsegs[0][0])
cHW_max = np.max(contour_data, axis=0)[1]
cHW_max_idx = np.argwhere(cHW_range >= cHW_max)[0,0]

#cHW_min_idx = np.argwhere(r_i_sys_rec_array > lambda_min)[0]
#plt.vlines(r_i_sys_range[lambda_min_idx], np.min(cHW_range), np.max(cHW_range), linestyles='dotted', color='black')
plt.hlines(cHW_max, np.min(r_i_sys_range), np.max(r_i_sys_range), linestyles='dotted', color='black')
plt.vlines(r_i_sys_rec_array[cHW_max_idx], np.min(cHW_range), np.max(cHW_range), linestyles='dotted', color='black')
...
# plot chi2 profile at lambda_min
#fig, ax = plt.subplots(figsize=(10,6))
plt.plot(r_i_sys_range, chi2_values[cHW_max_idx,:])
plt.axvline(r_i_sys_rec_array[cHW_max_idx],0,1, linestyle='dotted', color='black' )
plt.xlabel(r'$\lambda$')
plt.ylabel(r'$\chi^2$')
plt.show()
