...
# top flavour scheme
path_to_lhe_eft = '/Users/jaco/Documents/ML4EFT/MG5_aMC_v2.7.3/bin/pp_zh_eebb_cHDD/Events/run_01_0/unweighted_events.lhe'
path_to_lhe_sm = '/Users/jaco/Documents/ML4EFT/MG5_aMC_v2.7.3/bin/pp_zh_eebb_sm/Events/run_01_0/unweighted_events.lhe'
...
def read_events(path_to_lhe):
    ...
    for e in pylhe.readLHE(path_to_lhe):
        ...
        data_madgraph.append([d_eta_bb, d_phi_bb, d_R_bb, m_bb, pt_b1, pt_b2])
...
x_labels = [r'$|\Delta\eta(b_1, b_2)|$', r'$\Delta\phi(b_1, b_2)$', r'$\Delta R(b_1, b_2)$', r'$m_{b\bar{b}}\;[\mathrm{GeV}]$', r'$p_T^{b_1}\;[\mathrm{GeV}]$', r'$p_T^{b_2}\;[\mathrm{GeV}]$']
...
for i in range(n_cols * n_rows):
    ...
    hist_mg_sm, bins_mg = np.histogram(events_sm,
                                    bins=np.linspace(np.min(events_sm), np.max(events_sm), 30),
                                    density=True)
    #hist_mg_sm *= data_sm[0, 0]

    events_eft = data_eft[1:, i]
    hist_mg_eft, _ = np.histogram(events_eft,
                                       bins=np.linspace(np.min(events_eft), np.max(events_eft), 30),
                                       density=True)
    #hist_mg_eft *= data_eft[0, 0]

    plt.step(bins_mg[:-1], hist_mg_sm, c='C0', where='post', label= r'$\mathrm{SM}$')
    plt.step(bins_mg[:-1], hist_mg_eft, c='C1', where='post', label=r'$\mathrm{EFT}\;(\mathrm{cHD}=10)$')
    if i > 3:
        plt.xlim((np.min(events_sm), 500))
    else:
...
#plt.show()
plt.savefig('/Users/jaco/Documents/ML4EFT/plots/2022/31_01/decay_kinematics_bb_6.pdf')