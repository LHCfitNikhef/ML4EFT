...
# top flavour scheme
path_to_lhe_eft = '/Users/jaco/Documents/ML4EFT/code/dev/unweighted_events_sm.lhe'
...
def read_events(path_to_lhe):
    ...
    for e in pylhe.readLHE(path_to_lhe):
        ...
        data_madgraph.append([d_eta_bb, d_phi_bb, d_R_bb, m_bb, pt_b1, pt_b2])#, pt_z, d_phi_z_bb])
        cnt += 1
...
for i in range(n_cols * n_rows):
    ...
    plt.step(bins_mg[:-1], hist_mg_sm, c='C0', where='post', label= r'$\mathrm{SM}\;(\mu_R = \mu_F = m_Z)$')
    plt.step(bins_mg[:-1], hist_mg_eft, c='C1', where='post', label=r'$\mathrm{SM}\;\mathrm{dynamical\;scale}$')
    if i > 3:
        plt.xlim((np.min(events_sm), 750))
    else:
...
#plt.show()
plt.savefig('/Users/jaco/Documents/ML4EFT/plots/2022/04_02_meeting/decay_kinematics_fact_ren_scale_mz.pdf')