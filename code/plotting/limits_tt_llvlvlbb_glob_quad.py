...
paths_plot_5 = [samples_nn_quad_ptll, samples_binned_quad_ptll_etal, samples_nn_quad_ptll_etal]

labels_5 = [r"$\mathrm{Unbinned}\;\mathrm{ML}\;(p_T^{\ell\bar{\ell}})$",
            r"$\mathrm{Binned}\;(p_T^{\ell\bar{\ell}}, \eta_\ell)$",
            r"$\mathrm{Unbinned}\;\mathrm{ML}\;(p_T^{\ell\bar{\ell}}, \eta_\ell)$",
            r'$\mathrm{SM}$']
...
def ellipse_overview(coeff_dict, labels, paths, colors=None, alphas=None):
    ...
    for (c1, c2) in itertools.combinations(coeff_dict.keys(), 2):
        ...
        hndls = plotter.plot(ax, dfs, coeff1=c2, coeff2=c1,
                                 ax_labels=[coeff_dict[c2], coeff_dict[c1]],  kde=True, colors=colors, alphas=alphas)
        if row_idx != -1:
            ax.set(xlabel=None)
            ax.tick_params(
...
fig_5 = ellipse_overview(coeff_dict, labels_5, paths_plot_5, colors=['C6', 'C0', 'C1'], alphas=[0.25, 0.25, 0.5])
fig_5.savefig('/data/theorie/jthoeve/ML4EFT_jan/ML4EFT/plots/2022/23_10/tt_glob_quad_binned_nn_ptll_new_col.pdf')
